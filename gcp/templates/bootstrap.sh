...
gcloud compute disks create {{ STACK }}-manager-1-data \
  --type={{ properties['managerDiskType'] }} \
  --size={{ properties['managerDiskSize'] }}GB
...
gcloud compute instances create {{ STACK }}-manager-1 \
  ...
  --metadata infrakit--group={{ STACK }}-managers,infrakit--config_sha=bootstrap,infrakit-link=bootstrap \
  --metadata-from-file startup-script=/start.sh \
  --boot-disk-size {{ properties['managerBootDiskSize'] }}GB \
  --boot-disk-type {{ properties['managerDiskType'] }} \
  --boot-disk-device-name {{ STACK }}-manager-1 \
  --disk=name={{ STACK }}-manager-1-data
