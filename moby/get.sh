...
docker run --rm -v ${PWD}/tmp:/tmp -e AWS_ACCESS_KEY_ID=$QA_AWS_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY=$QA_AWS_SECRET_ACCESS_KEY docker4x/awscli aws s3 cp ${MOBY_IMG_URL} /tmp/${MOBY_IMG_NAME} 
pushd tmp
