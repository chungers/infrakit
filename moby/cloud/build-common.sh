...
configure_syslinux_on_device_partition()
{
	...
	ROOT_PARTITION_MOUNT="${MOBY_OUTPUT}/moby"
	...
	cp ${MOBY_OUTPUT}/syslinux.cfg ${ROOT_PARTITION_MOUNT}
	cat ${ROOT_PARTITION_MOUNT}/syslinux.cfg
	cp ${MOBY_OUTPUT}/vmlinuz64 ${ROOT_PARTITION_MOUNT}
	cp ${MOBY_OUTPUT}/initrd.img ${ROOT_PARTITION_MOUNT}
