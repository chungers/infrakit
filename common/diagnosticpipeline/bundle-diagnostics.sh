...
IMAGE="nathanleclaire/diagnostics"
...
process()
{
        ...
        do
                ...
                debugecho "cp \"${HOST}\" \"${TMPDIR}/${REPORT}\""
                cp "${HOST}" "${TMPDIR}/${REPORT}"

                # Note the use of ADD here.  This untars the archive
                # automatically.
                echo "ADD ${REPORT} /info/${HOSTNAME}" >>"${DF}"
        done
...
do
        ...
        aws s3 sync --quiet --delete s3://editionsdiagnostics "${BUCKET}"
