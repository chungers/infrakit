...
docker run --rm --name=cloud_registration -v /var/run/docker.sock:/var/run/docker.sock -e DOCKER_USER -e DOCKER_PASS -e SWARM_NAME -e INTERNAL_ENDPOINT="$LB_IP" -e DOCKERCLOUD_REST_HOST -e JWT_URL -e JWK_URL docker4x/cloud-azure:$DOCKER_FOR_IAAS_VERSION
fi
